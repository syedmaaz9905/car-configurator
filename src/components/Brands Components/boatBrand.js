...
const BoatBrand = () => {
  ...
  const goToBoatSeries = (brandId) => {
    navigate(`/boat/${brandId}/series`);
  };
  ...
  return (
    <div>
      ...
      <div className='flex flex-wrap justify-center gap-8'>
        {boatBrands.map((brand, index) => (
          <div
            key={index}
            className='bg-white rounded-lg overflow-hidden shadow-xl hover:shadow-xl hover:translate-y-1 hover:flip boatBrandCards'
            style={{ minHeight: '200px', padding: '12px', display: 'flex', alignItems: 'center' }}
            onClick={() => goToBoatSeries(brand.id)}
          >
            <img
              src={brand.logo}
              alt={`${brand.name} Logo`}
